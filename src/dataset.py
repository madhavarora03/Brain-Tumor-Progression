...
class BraTSPRODataset(Dataset):
    def __init__(self, root_dir, patients_json, target_shape=(128, 128, 128), use_registered=True, use_seg=False,
                 transform=None):
    ...
    def __getitem__(self, idx):
        ...
        diff_vol = followup_vol - baseline_vol

        x = np.concatenate([baseline_vol, followup_vol, diff_vol], axis=0)  # Shape: (12, H, W, D)
    ...
    def _load_mri_modalities(self, folder):
        channels = []
...
def test():
    dataset = BraTSPRODataset(root_dir="data", patients_json="data/patients.json", use_registered=True)
    print(len(dataset))
